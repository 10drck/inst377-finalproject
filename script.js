...
function injectHTML(list) {
  ...
  const target = document.querySelector('#uni_list');
  target.innerHTML = '';
  list.forEach((item) => {
...
async function mainEvent() { // the async keyword means we can make API requests
  ...
  loadDataButton.addEventListener('click', async (submitEvent) => { // async has to be declared on every function that needs to "await" something
    ...
    const results = await fetch('http://universities.hipolabs.com/search?country=United%20States');
  ...
  generateListButton.addEventListener('click', (event) => {
    ...
    const unisList = cutUniList(currentList)
    injectHTML(unisList)
  })
}
...
Highcharts.chart('container', {
  ...
  title: {
      text: 'Universities in the United States'
  },

  subtitle: {
      text: 'Source:<a href="https://github.com/Hipo/university-domains-list">University API</a>'
  },
